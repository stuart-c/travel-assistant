...
- **Journeys Configuration**: Dedicated configuration page at `/config/journeys` with an interactive Grid.js table and 2-tab modal dialogue (**Journey Details** and **Calculated Routes**) for configuring travel journeys between rail stations, bus stops, Home Assistant locations, and custom locations with multi-time-window scheduling. Features an interactive `vis-network` vertical DAG diagram visualizing discovered multi-modal corridors, intermediate stops, mode-coloured transit edges, rich hover tooltips, and fit controls, automatically clearing stale routes when journey parameters are modified, and automatically queuing targeted walking discovery when location endpoints change, bus timetable downloads when bus stop endpoints change, and background route discovery.
...
- **Database Statistics & Backup**: SQLite storage usage and schema table metrics view at `/config/db` displaying database disk size, record counts per table, on-demand SQLite database file download (`GET /config/db/download`), manual refresh button, and automated 1-minute background refresh.
- **Background Synchronisation**: Dedicated dashboard at `/config/sync` with interactive Grid.js table displaying cached transit datasets (Bus Routes, NaPTAN Transit Stops, Stop Interchanges, Home Assistant Locations, National Rail Darwin Train Timetables, Walking Connections, BODS Bus Timetables, Journey Routes), last updated timestamps, status badges, manual refresh button, automated 1-minute background refresh, and on-demand refresh triggers.
- **Automated Background Updates**: Background daemon worker (`SyncWorker`) that automatically evaluates and synchronises transit datasets, Home Assistant zones, National Rail Darwin train timetables, walking connections, BODS bus timetables, and journey route calculations in dependency order whenever data is overdue or requested on demand.
...
- **Journey Departure Detection & Mobile Notification Dispatcher**: Background daemon monitor (`DepartureMonitor`) running every 30 seconds that detects Stuart near the start of any configured journey (`person.stuart` within 200 metres GPS distance or inside the origin Home Assistant zone) during active scheduled time windows, evaluates upcoming transit departures using the RAPTOR engine and live feeds (Darwin/BODS), and dispatches rich departure alerts to Stuart's mobile device (`notify.mobile_app_stuart_mobile` with no fallback) exactly 15 minutes before the calculated leave time ($T_{\text{leave}} - 15\text{ mins}$, factoring walking duration to the departure transit stop). Supports both `depart` mode (opening evaluation 30 minutes prior to scheduled departure) and `arrive` mode (dynamically calculating maximum journey duration from route templates to open evaluation well in advance of target arrival time with a 45-minute buffer). If Stuart does not depart as expected and remains near the start past the leave time, subsequent departure services are automatically evaluated and alerted 15 minutes in advance to minimise waiting time once the journey begins.
...
- **Optional Model Context Protocol (MCP) Server**: Streamable HTTP and Server-Sent Events (SSE) server on port `8098` exposing rich transit intelligence and database tools (journey management, timetable scheduling, walking links, stops lookup, live departure boards, dispatcher controls, sync triggers, schema table inspection, and granular SQL querying) directly to AI coding assistants and autonomous agents. Native Streamable HTTP supports JSON-RPC POST method calls, GET SSE streams, and DELETE session termination seamlessly across `/sse`, `/mcp`, and `/`. Configured to permit connections from local area network (LAN) clients and Home Assistant reverse proxy requests by default, with optional host restriction via `MCP_ALLOWED_HOSTS`.
- **MCP Tools Configuration & Access Control**: Dedicated configuration page at `/config/mcp` with an interactive Grid.js table, modern toggle switch controls, and differential changeset persistence for managing per-tool binary access status (`disabled` / `enabled`). Features strict opt-in security (newly discovered tools default to disabled), dynamic tool catalogue filtering, read-only evaluation for dispatcher checks, and dedicated tool separation for database querying (`db_query` strictly read-only for `SELECT` queries, and `db_execute` for mutating `INSERT`, `UPDATE`, and `DELETE` commands).
