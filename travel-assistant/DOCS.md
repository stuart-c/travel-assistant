...
- **Background Synchronisation**: Dedicated dashboard at `/config/sync` with interactive Grid.js table displaying cached transit datasets (Bus Routes, Bus Stops, Train Stations, Home Assistant Locations), last updated timestamps, status badges, and on-demand refresh triggers.
