...
- **Background Synchronisation**: Dedicated dashboard at `/config/sync` with interactive Grid.js table displaying cached transit datasets (Bus Routes, NaPTAN Transit Stops, Home Assistant Locations, National Rail Darwin Train Timetables, Walking Connections), last updated timestamps, status badges, and on-demand refresh triggers.
- **Automated Background Updates**: Background daemon worker (`TransitBackgroundWorker`) that automatically synchronises transit datasets, Home Assistant zones, National Rail Darwin train timetables, and walking connections whenever data is older than 24 hours (1 day).
