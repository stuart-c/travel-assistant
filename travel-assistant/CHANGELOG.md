...
- Fixed Journey Calculated Routes Directed Acyclic Graph (DAG) viewer layout, transit modes, and stop icons:
  ...
  - Deduplicated multiple identical station and bus stop nodes (including directional indicators such as `(S-bound)`, `(N-bound)`, `(Southbound)`, position indicators `(opp)`, `(adj)`, `(nr)`, and bay/stand indicators) into unified transit access nodes based on canonical stop names, eliminating orphan disconnected nodes and standardising transport mode icons (`🚆`, `🚌`, `🚇`, `🚋`, `⛴️`, `✈️`, `🚏`) so that bus stations display bus emoji (`🚌`) rather than rail emoji (`🚆`).
