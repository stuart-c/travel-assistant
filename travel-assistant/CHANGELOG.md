...
### Fixed
- Fixed Journey Calculated Routes Directed Acyclic Graph (DAG) viewer layout and stop duplication:
  - Enforced strict top-to-bottom DAG hierarchy by computing explicit topological levels (`level`) across route corridors using longest-path relaxation, pinning the origin node at the top (`level: 0`) and destination node at the bottom (`level: destLevel`), with all directed arrows flowing downwards.
  - Deduplicated multiple identical station and bus stop nodes (such as multiple "Bus station" bays or "Stevenage Rail Station" variations) into unified transit access nodes based on canonical stop names, standardising transport mode icons (`🚆`, `🚌`, `🚇`, `🚋`, `⛴️`, `✈️`, `🚏`).
  - Optimised Vis.js hierarchical layout configuration (`blockShifting: true`, `edgeMinimization: true`, `parentCentralization: true`, `treeSpacing: 260`, `nodeSpacing: 220`) to fan out diverse route branches sideways cleanly while minimising edge crossings, and aggregated multi-line transit edges into consolidated links with rich hover tooltips.
