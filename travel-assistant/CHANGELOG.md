...
- Fixed Journey Calculated Routes Directed Acyclic Graph (DAG) viewer layout, transit modes, and stop icons:
  ...
  - Deduplicated multiple identical station and bus stop nodes (such as directional variations `Emperors Gate (opp)` vs `Emperors Gate`, and bay/stand indicators) into unified transit access nodes based on canonical stop names, standardising transport mode icons (`🚆`, `🚌`, `🚇`, `🚋`, `⛴️`, `✈️`, `🚏`) and ensuring bus stations display bus emoji (`🚌`) rather than rail emoji (`🚆`).
