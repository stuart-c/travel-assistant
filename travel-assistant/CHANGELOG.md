...
- Added modal sequence validation rules to Mode 1 Route Generator (`find_routes` and `prune_route_templates`):
  ...
  - **Maximum of 3 Consecutive Legs of Same Mode**: Enforced a strict maximum of 3 consecutive legs sharing the identical transport mode in a row (e.g. 4 or more buses or trains in a row rejected unless separated by an intervening mode).
...
### Fixed
- Fixed Journey Calculated Routes Directed Acyclic Graph (DAG) viewer layout, vertical gaps, and stop duplication:
  - Enforced strict top-to-bottom DAG hierarchy by computing normalized topological progression along route corridors, assigning forward-only levels, and applying consecutive level compaction to eliminate vertical voids and large empty gaps between origin and destination nodes.
